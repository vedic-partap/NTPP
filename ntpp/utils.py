...
def extractor(path, minl):
    ...
    filtered_event_time_stamps = {
        k: v
        for k, v in event_time_stamps.items()
        if len(event_time_stamps[k]) < minl
    }
    with open(path + '/events.txt' 'w') as f:
        for k in filtered_event_time_stamps.keys():
            f.write(" ".join([x[0] for x in filtered_event_time_stamps[k]]))
            f.write("\n")

    with open(path + '/times.txt', 'w') as f:
        for k in filtered_event_time_stamps.keys():
            f.write(" ".join([x[1] for x in filtered_event_time_stamps[k]]))
            f.write("\n")
